...
import com.utrobin.luna.utils.Utils
...
//        }
    ...
    private fun fillToolbar() {
        master.avatar?.path?.takeIf { it.isNotBlank() }
                ?.let {
                    Glide.with(this)
                ...
                ?: avatar.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.no_avatar))

        masterName.text = master.name + "sdasadsadadasd" ?: throw NullPointerException("Master has not name!")
        role.text = "Ведущий стилист"
        salonName.text = "Many Hands"
        fillRating()
    }

    private fun fillRating() {
        val params = LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT)
        params.setMargins(0, 0, resources.getDimension(R.dimen.master_activity_space_between_stars).toInt(), 0)
        Utils.fillContainerWithSmallStars(this, master.stars, starsContainer, params)
        ratesCount.text = resources.getQuantityString(R.plurals.rates_count, master.ratesCount, master.ratesCount)
    }
